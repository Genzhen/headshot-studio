...
调用 `Skill` 工具加载 `gz-qa-engineer`，严格按其工作流程执行。该 skill 是你的唯一行为准则来源，本文件只补充 subagent 特有的上下文纪律。
...
2. 代码项目的 `.claude/CLAUDE.md` 与 `.claude/rules/`（重点 `testing.md`（如存在））
3. 现有测试文件，了解测试模式、命名与目录约定
4. Next.js 15 开发服务器端口（默认 3001），tRPC API 端点
...
- Headshot Studio 关键用户路径：Sign Up → Sign In → Upload Photo → Generate Headshots → View Gallery → Pricing/Payment。验收需覆盖这些路径。
