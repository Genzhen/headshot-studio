...
- **React 规范**: 是否正确区分 Server/Client Component、hooks 使用是否规范
...
- **性能**:
  ...
  - N+1 查询（Prisma 未用 include/select）
  - 大循环、内存泄漏风险（未清理的 effect、未取消的请求）
- **Lambda 适配**: 是否有违反无状态的设计（本地文件持久化、内存缓存）
- **可维护性**: 代码是否易于理解和修改

### 4. 安全扫描

基于 Headshot Studio 的安全要求逐项扫描：
...
```
🚨 安全扫描警告 — Headshot Studio / {feature-name}
