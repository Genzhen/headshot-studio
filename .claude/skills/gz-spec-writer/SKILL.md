...
Headshot Studio 已确认为 **Monorepo** 架构：

- **Monorepo 工具**: Turborepo + pnpm workspaces
- **Apps**: `apps/web` (Next.js 前端)
...
- 验收标准（怎样算完成）
- 约束条件（性能、安全、Lambda 适配）
- 依赖（外部服务：AI 图像生成、S3 存储、Polar 支付、Neon DB）
...
## 依赖
- {外部服务/库 — 如 Replicate API、S3、Polar}
...
## 数据模型
{新增/修改的 Prisma schema — 注意 Serverless 适配}

## API 契约
{tRPC procedure 定义 — router名.procedure名、输入 zod schema、输出类型}

## 组件设计
{新增/修改的 React 组件 — 标注 Server/Client Component}
...
- [ ] T-001: {schema 变更/migration} `packages/db/prisma/schema/{file}.prisma` ~{预估时间}
...
- [ ] T-004: {页面/组件} `apps/web/src/app/{route}/page.tsx` ~{预估时间}
