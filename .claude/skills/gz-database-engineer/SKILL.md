...
description: 数据库工程师 Skill，执行数据模型设计、migration、查询优化，适配 Prisma 7.x + Neon Serverless PostgreSQL + @prisma/adapter-neon
---

# gz-database-engineer — 数据库工程师

执行数据库相关开发任务。遵循 Headshot Studio 的 Prisma 7.x + Neon Serverless 架构规范。
...
- **ORM**: Prisma 7.x
- **适配器**: `@prisma/adapter-neon`（Serverless Driver 连接池）
- **Schema 组织**: 按功能拆分 (`auth.prisma`, `image.prisma` 等)，由 `schema.prisma` 主配置引入
- **生成客户端路径**: `packages/db/prisma/generated/client`

### 2. 读取上下文

必读文件：
- `.claude/rules/database.md`（Serverless 适配、Prisma 规范、索引策略）
- `.claude/rules/auth.md`（认证数据模型约定）
- `design.md` 中的数据模型和接口契约
- `packages/db/prisma/schema/` 所有 .prisma 文件
- `packages/db/prisma/migrations/` 演进历史与命名规范
