...
export default function PricingCard({ plan, onSelect }: PricingCardProps) {
  return (
    <div
      className={`relative flex h-full flex-col rounded-[1.75rem] border-2 p-8 shadow-[0px_4px_20px_rgba(10,37,64,0.06)] transition-all ${
        plan.isPopular
    ...
    >
      ...
      <button
        ...
        className={`w-full rounded-xl py-4 text-label-sm font-bold transition-all active:scale-95 ${
          plan.isPopular
            ? "bg-secondary-container text-primary shadow-lg hover:brightness-110"
